...
var processors = {
  css: function(fileContents, className) {
    var parsedCss = css.parse(fileContents);
    var transformedParsedCSS = transformCSS(className, parsedCss);
...
function indentStyles(fileContents, className) {
...
function wrapStyles(fileContents, className) {
  // Replace instances of :--component with '&'
  fileContents = fileContents.replace(/:--component/g, '&');

  // Wrap the styles inside the generated class
  return '.' + className + '{' + fileContents + '}';
}

function transformCSS(className, parsedCss) {
  ...
  rules.forEach(function(rule) {
    rule.selectors = rule.selectors.map(function(selector) {
      ...
      if (selfSelectorMatch) {
        return selector.replace(selfSelectorMatch[0], '.' + className);
      } else {
  ...
  return readTree(this.inputTree).then(function(srcDir) {
    ...
    for (var i = 0, l = paths.length; i < l; i++) {
      ...
      if (filepath[filepath.length-1] !== '/') {
        ...
        // Get cached className or create a new one
        var podClassName = podPath.join('-');

        var className;

        if (podClassName in guidCache) {
          className = guidCache[podClassName];
        } else {
          className = guidCache[podClassName] = podClassName;
        }
